...
namespace ScriptLib
{
    ...
    public static class Parser
    {
        ...
        public static SyntaxNode parse(List<Token> toks, Context context = null)
        {
            if (toks.Count > 0)
            {
                tokens = toks;
        ...
        public static ParseResult atom(Context context = null)
        {
            ...
            else if (curTok.matches(TOKENS.TT_KEYWORD, "import"))
            {
                advance();
                result.registerAdvancement();
                System.IO.StreamReader sr = new System.IO.StreamReader(curTok.value);
                while(sr.EndOfStream == false)
                {
                    string importedLine = sr.ReadLine();
                    List<Token> importedTokens = Lexer.processLine(importedLine);
                    SyntaxNode importedNode = parse(importedTokens);
                    RunTimeResult importedResult = Interpreter.visitNode(importedNode, context);
                    //result = result.register(funcDef());
                    if(importedResult.error != null)
                    {
                        result.failure(new ImportError(importedResult.error.details, context, 
                            importedResult.error.pos.fileName, importedResult.error.pos.fileText, 
                            importedResult.error.pos.Line, importedResult.error.pos.pos.IX,
                            importedResult.error.pos.pos.IX));
                    }
                    if(result.error != null)
                    {
                        return result;
        ...
        */
        ...
        public static ParseResult importFunc()
        {
            ParseResult result = new ParseResult();
            Token opTok = curTok;
            Token funcName = null;
